...
Modified by Vivek Kale, February 28, 2022
...
#pragma omp target data map(alloc: temp_in[0:ni*nj], temp_out[0:ni*nj]) 
{
...
#pragma omp target data map(to:temp1[0:(rows+2)*LDA], temp2[0:(rows+2)*LDA]) device(chosenDev)
  {
	for(istep=0; istep < nstep; istep++)
	{
        ...
        {
          #pragma omp target update map(from:temp2[rows*LDA:LDA]))
        }

        /*update the upper halo to the host except the first device*/
        if(tid != 0)
        {
           #pragma omp target update map(from:temp2[LDA:LDA])
        }
        ...
        if(tid != 0)
        {
          #pragma omp target update map(to:temp2[0:LDA])
        }
        
        if(tid != NUM_THREADS-1)
	  { 
#pragma omp target update map(from: temp2[(rows+1)*LDA:LDA])
	  }
	...
	// #pragma acc update host(temp1[LDA:rows*LDA])
        #pragma omp target update map(from:temp1[LDA:rows*LDA])
